#!/usr/bin/env python3
"""Standalone regression for generated-audio ownership across a prelude seam."""
...
PACKAGE = "h3_audio_prelude_overlap_ownership_unit"
...
def main() -> None:
    ...
    try:
        ...
        extension = {
            "waveform": torch.full((1, 2, 3), 2.0),
            "sample_rate": 24,
        }

        def ownership_load(path: str) -> dict:
            if path == "prelude.safetensors":
                return {"waveform": torch.full((1, 2, 5), 1.0)}
            if path == "clip_0001.safetensors":
                return {"overlap_audio": torch.full((1, 2, 5), 9.0)}
            raise AssertionError("unexpected safetensors path: %s" % path)

        chain._st_load = ownership_load
        owned = chain._generated_audio_with_prelude(
            extension, 3, _prelude(), _manifest())
        ...
        chain._st_load = legacy_load
        legacy = chain._generated_audio_with_prelude(
            extension, 3, _prelude(), _manifest())
    ...
    print("H3 audio prelude ownership: scene 1 replaces the protected prelude tail")
