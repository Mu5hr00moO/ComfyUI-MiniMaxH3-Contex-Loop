...
def _require_raw_guide_mask_support() -> None:
...
def apply_raw_guide_prefix(
    ...
    if frames < 5:
        raise ValueError(
            "h3_raw_guide: raw masked AV continuation needs at least 5 "
            "context frames."
        )
    ...
    if covered_frames != frames:
        raise RuntimeError(
            "h3_raw_guide: raw video prefix covers "
    ...
    _LOG.info(
        "h3_raw_guide: continuation source=previous sampled raw AV latent; "
